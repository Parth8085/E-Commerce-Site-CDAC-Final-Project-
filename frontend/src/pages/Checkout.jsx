...
import { CreditCard, Smartphone, Banknote, Package } from 'lucide-react';
...
const Checkout = () => {
    ...
    const validatePayment = () => {
        ...
        // Razorpay and COD don't need validation here
    ...
    const handlePlaceOrder = async () => {
        ...
        // Legacy payment methods (Card, COD, etc.)
        setLoading(true);
        try {
        ...
        try {
            ...
            const options = {
                ...
                handler: async function (response) {
                    ...
                    setLoading(true);
                    try {
                        const checkoutResponse = await api.post('/order/checkout', {
                            shippingAddress,
                            shippingCity,
                            shippingState,
                            shippingZipCode,
                        ...
                        if (checkoutResponse.data.paymentStatus === 'Success') {
                            clearCart();
                            navigate(`/order-success/${checkoutResponse.data.orderId}`, {
                                state: { orderData: checkoutResponse.data }
                            });
                        } else {
                        ...
                        alert('Failed to complete order: ' + (typeof errorMsg === 'object' ? JSON.stringify(errorMsg) : errorMsg));
                    } finally {
                        setLoading(false);
                    }
                },
            ...
            if (error.response?.status === 401) {
                ...
                const errorMsg = error.response?.data || error.message;
                alert('Failed to initialize payment: ' + (typeof errorMsg === 'object' ? JSON.stringify(errorMsg) : errorMsg));
            }
            setLoading(false);
        }
    };
                    ...
                    {step === 2 && (
                        <div className="product-card" style={{ padding: '2rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                                {[
                                    { value: 'Razorpay', icon: CreditCard, label: 'Razorpay' },
